define(['baseView', 'dom', 'globalize', 'require', 'emby-linkbutton', 'emby-button', 'listViewStyle', 'paper-icon-button-light'], function (BaseView, dom, globalize, require) {
    ...
    function getEntryEditorJsPath() {
        return Dashboard.getConfigurationResourceUrl('prowlnotificationeditorjs');
    }

    function getDefaultEntry() {
        return ApiClient.getJSON(ApiClient.getUrl('ProwlNotifications/Default'));
    }
    ...
    function getConfiguredNotifications() {
        return ApiClient.getNamedConfiguration('prowlnotifications').then(function (config) {
    ...
    function deleteEntry(view, button) {
        ...
        return require(['confirm']).then(function (responses) {
            ...
            return confirm({
                ...
                ApiClient.ajax({
                    url: ApiClient.getUrl('ProwlNotifications', { Id: id }),
                    type: 'DELETE'
                }).then(function () {
    ...
    function getEntryHtml(entry, allEventTypes) {
        ...
        html += '<h3 class="listItemBodyText">' + (entry.FriendlyName || 'My Prowl') + '</h3>';
